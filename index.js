...
stories.forEach((story, index) => {
    story.addEventListener('click', () => {
        currentStory = index;
        viewerImg.src = story.dataset.src;
...
// next
document.querySelector('.next-story').onclick = () => {
    currentStory = (currentStory + 1) % stories.length;
    viewerImg.src = stories[currentStory].dataset.src;
};

// previous
document.querySelector('.prev-story').onclick = () => {
...
// CLICK IMAGE (ONLY FEED IMAGES)
document.querySelectorAll('.feeds .photo img').forEach(img => {

    img.addEventListener('click', () => {

        const parentPhoto = img.closest('.photo');

        currentImages = Array.from(parentPhoto.querySelectorAll('img'));
